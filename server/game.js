...
Meteor.methods({
  ...
  submitAnswers: function(playerId, gameId, answers) {
    ...
    for (var i = 0, len = submittedAnswers.length; i < len; i++) {
      if (submittedAnswers[i].player_id == playerId) {
        submitted = true;
        break;
      }
    }
  ...
  submitJudgment: function(playerId, gameId, rejected) {
    ...
    for (var i = 0, len = judgments.length; i < len; i++) {
      if (judgments[i].player_id == playerId) {
        judged = true;
        break;
      }
    }
...
Meteor.startup(function () {
  ...
  for (var i = 0, len = activeGames.length; i < len; i++) {
    setGameInterval(activeGames[i]._id);
  }
});
